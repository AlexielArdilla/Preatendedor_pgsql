...
function confirmarCrear() {
	...
	return confirm("Está seguro que desea Crear este Trámite?");
}
</script>
...
tinymce.init({
  ...
  <body>
	...
	<form accept-charset="UTF-8" role="form" action="ControladorAltaTramite" method="post" onSubmit="return confirmarCrear()">
  ...
  <div class="form-group">
    ...
    <c:forEach items="${ventanillas}" var="ventanillas">
      <option id="${ventanillas.id}" value="${ventanillas.id}">${ventanillas.nombre}</option>
      </c:forEach>
    </select>
  </div>
  ...
  <div class="form-group">
    ...
    <select class="form-control" name="Tipo_tramite" required>
      ...
      <option id="${tramites.id}" value="${tramites.id}">${tramites.tipo}</option>
      </c:forEach>
    </select>
  </div>
