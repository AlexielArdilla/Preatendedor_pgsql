...
public class gestionDAO {
	...
	public int agregarGestion(Gestion gest){
		...
		String sql = "insert into gestion values(?, ?, ?, now())";
		...
		try{
		 ...
		 r = 0;
	    	...
	    	s.setInt(1, traerUltimoIdGestion()+1);
	        s.setString(2, gest.getNom_apel());
	        s.setString(3, gest.getNom_tramite());
	...
	public int traerUltimoIdGestion(){
		
		int id_ult_gestion = 0;
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			con = MiConexion.getConnection();
			ps = con.prepareStatement(
					"select max(id) from gestion");//***********
			rs = ps.executeQuery();
			
			if (rs.next()) {

				id_ult_gestion = rs.getInt(1);
